...
void main() {
    group('Simplify', () {
        test('simplifies points correctly with the default tolerance', () {
        ...
        test('simplifies points correctly with the given tolerance', () {
            ...
            expect(result.corner, equals(corner));
            expect(result.path, equals(simplified5));
        });

        test('return a line that have only have had its padding removed if it has only two points', () {
            var result = simplify('1, 2 4, 5');
            expect(result.corner, equals(new Point()..x=1..y=2));
            expect(result.path, equals('M 0.0 0.0 L 3.0 3.0'));
        });

        test('returns a empty result if it has no points', () {
            var result = simplify('');
            expect(result.path, equals(null));
        });
    });
}
